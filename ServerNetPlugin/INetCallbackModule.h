...
public:
	...
	virtual void AddHttpCallback(const std::string& path, std::shared_ptr<IHttpBindFunc> callback) = 0;
	virtual const std::map<std::string, std::shared_ptr<IHttpBindFunc>>& GetHttpCallbackMap() = 0;
};
