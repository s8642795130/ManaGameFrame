...
private:
	...
	std::map<std::string, std::shared_ptr<IHttpBindFunc>> m_http_callback;
public:
	...
	virtual void AddHttpCallback(const std::string& path, std::shared_ptr<IHttpBindFunc> bind_func);
	virtual const std::map<std::string, std::shared_ptr<IHttpBindFunc>>& GetHttpCallbackMap();
};
