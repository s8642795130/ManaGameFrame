...
private:
	...
	std::map<int, std::function<void(IClientNetActor&)>> m_master_callback;
...
public:
	...
	virtual void AddMasterCallback(const int msg_id, std::function<void(IClientNetActor&)> callback);
	virtual const std::map<int, std::function<void(IClientNetActor&)>>& GetMasterCallbackMap();
