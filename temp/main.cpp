...
void init_alias() { alias_map.emplace(std::pair<std::string, std::string>("ll", "ls -l")); }

// history
std::vector<std::string> cmd_history;

// panic
void panic(std::string hint, bool exit_ = false, int exit_code = 0)
{
    if (SHOW_PANIC)
    {
        std::cerr << "[!ExpShell panic]: " << hint << std::endl;
    }
...
std::vector<std::string> string_split_protect(const std::string& str, const std::string& delims) {
    std::vector<std::string> vec;
    std::string tmp = "";
...
std::string string_split_last(const std::string& s, const std::string& delims)
{
    std::vector<std::string> split_res = string_split(s, delims);
    return split_res.at(split_res.size() - 1);
}

std::string string_split_first(const std::string& s, const std::string& delims)
{
    std::vector<std::string> split_res = string_split(s, delims);
...
void check_wait_status(int& wait_status)
{
    if (WIFEXITED(wait_status) == 0)
    {
        ...
        if (SHOW_WAIT_PANIC)
        {
            panic("child exit with code " + std::string(buf));
        }
    }
}
...
public:
    ...
    exec_cmd(std::vector<std::string>& argv) {
        this->type = CMD_TYPE_EXEC;
        this->argv = std::vector<std::string>(argv);
    }
};
...
public:
    ...
    redirect_cmd(int type, cmd* cmd_, std::string file, int fd)
    {
        this->type = type;
        this->cmd_ = cmd_;
        this->file = file;
...
cmd* parse_exec_cmd(std::string seg) {
    seg = trim(seg);
    std::vector<std::string> argv = string_split_protect(seg, WHITE_SPACE);
    ...
    while (i < line.length()) {
        if (line[i] == '<' || line[i] == '>') {
            ...
            while (j < line.length() && !is_symbol(line[j]))
                j++;
            std::string file = trim(line.substr(i + 1, j - i));
...
int process_builtin_command(std::string line) {
    ...
    if (line == "cd") {
        ...
        std::string arg1 = string_split(line, WHITE_SPACE)[1];
        if (arg1.find("~") == 0)
    ...
    // 2 - quit
    if (line == "quit") {
        std::cout << "Bye from ExpShell." << std::endl;
        exit(0);
    }
    // 3 - history
    if (line == "history") {
        for (int i = cmd_history.size() - 1; i >= 0; i--)
            std::cout << "\t" << i << "\t" << cmd_history.at(i) << std::endl;
        return 1;
    }
...
void run_cmd(cmd* cmd_) {
    ...
    case CMD_TYPE_EXEC: {
        ...
        if (alias_map.count(ecmd->argv[0]) != 0) {
            std::vector<std::string> arg0_replace =
                string_split(alias_map.at(ecmd->argv[0]), WHITE_SPACE);
            ecmd->argv.erase(ecmd->argv.begin());
            for (std::vector<std::string>::reverse_iterator it = arg0_replace.rbegin();
        ...
        for (int i = 0; i < ecmd->argv.size(); i++) {
            std::string arg_trim = trim(ecmd->argv[i]);
